...
exports.find = async (filterObject) => {
    try {
        const resultObject = await fieldModel.findOne(filterObject).lean();
...
exports.list = async (filterObject, selectionObject, sortObject, pageNumber, limitNumber) => {
    try {
        ...
        const resultArray = await fieldModel.find(filterObject).lean()
            .populate({ path: "requestedBy", select: "userName image" })
            .sort(sortObject)
            .select(selectionObject)
            .limit(limitNumber)
        ...
        const count = await fieldModel.count(filterObject);
        return {
            success: true,
            code: 200,
            result: resultArray,
...
exports.create = async (formObject) => {
    try {
        ...
        const resultObject = new fieldModel(formObject);
...
exports.update = async (_id, formObject) => {
    try {
        ...
        const resultObject = await fieldModel.findByIdAndUpdate({ _id }, formObject, { new: true, select: "-password" });
...
exports.remove = async (_id) => {
    try {
        ...
        const resultObject = await fieldModel.findByIdAndUpdate({ _id }, { isActive: false });
        if (!resultObject) return {
            success: false,
            code: 500,
            error: i18n.__("internalServerError")
