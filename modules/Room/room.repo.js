...
exports.list = async (filterObject, selectionObject, sortObject, pageNumber, limitNumber) => {
    try {
        ...
        const resultArray = await roomModel.find(filterObject).lean()
            .populate({ path: "customer", select: "name image" })
            .populate({ path: "seller", select: "userName image" })
            .populate({ path: "messages.admin", select: "name image" })
            .sort(sortObject)
        ...
        const count = await roomModel.count(filterObject);
        return {
            success: true,
            code: 200,
            result: resultArray,
...
exports.create = async (formObject) => {
    try {
        ...
        const resultObject = new roomModel(formObject);
...
exports.update = async (_id, formObject) => {
    try {
        ...
        const resultObject = await roomModel.findByIdAndUpdate({ _id }, formObject, { new: true })
...
exports.remove = async (_id) => {
    try {
        const resultObject = await roomModel.findByIdAndDelete({ _id })
