...
exports.find = async (filterObject) => {
    try {
        const resultObject = await basketModel.findOne(filterObject).lean();
...
exports.get = async (filterObject, selectionObject) => {
    try {
        ...
        if (!resultObject)
            resultObject = await basketModel.findOneAndUpdate(filterObject,
...
exports.updateWithFilter = async (filterObject, formObject) => {
    try {
        const resultObject = await basketModel.findOneAndUpdate(filterObject, formObject, { new: true })
        if (!resultObject) return {
            success: false,
            code: 404,
            error: i18n.__("notFound")
...
exports.updateDirectly = async (_id, formObject) => {
    try {
        const resultObject = await basketModel.findByIdAndUpdate({ _id }, formObject, { new: true })
        if (!resultObject) return {
            success: false,
            code: 404,
            error: i18n.__("notFound")
...
exports.remove = async (_id) => {
    try {
        const resultObject = await basketModel.findByIdAndDelete({ _id })
